 ...
 * Disintegrate a int number, returns an array, works on pointers
 */
...
// Function to print the basic header
void sayHello(){
  printf("Using custom header file BCA.H ver %.1f\n", HEADER_VERSION);
  printf("Created and maintained by Nick\n\n");
}
...
// Disintegrate a given integer, return the result as an array
int *disIntegrate(int numToReverse, int numSize){
    int lastNum;
    numSize--;
    static int reversed[100]; // To advocate local address out of scope, static declaration is required
    while (numToReverse != 0){
        lastNum = numToReverse % 10;
        reversed[numSize] = lastNum;
        numSize--;
        numToReverse /= 10;
    }
    return reversed;
...
// Function to print a star pyramid of n rows
void printStarPyramid(int rows){
  int count = 1; // Number of stars initially
  for (int i = 1; i <= rows; i++) { // Handles the number of rows
    for (int j = rows; j > i; j--) { // Handles the spaces
      printf(" ");
    }
    for (int stars = 1; stars <= count; stars++) { // Handles the stars
      printf("*");
    }
    count += 2; // By 2 as we are adding one star to each side (left and right)
    printf("\n"); // Break the line!
  }
}
...
void printStarPyramidReversed(int rows){
  ...
  for (int i = 1; i <= rows; i++) { // Handles the rows
    ...
    count-=2; // Lessen the stars as this is a reverse pyramid
    printf("\n"); // Break the line!
  }
}
